...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build fake-only CLIP patch cache for DF40_test_cdf.")
...
def infer_spec(method_root: Path, relative_to: Path) -> tuple[str, str, str]:
...
def main() -> None:
    ...
    split, group, method = infer_spec(args.method_root, args.relative_to)
    if split != "DF40_test_cdf":
        raise ValueError(f"Expected split DF40_test_cdf, got {split}")
    ...
    print(
        f"[START] prepare_cdf_patch_clip\n"
        f"  split={split}\n"
        f"  group={group}\n"
        f"  method={method}\n"
    ...
    progress = tqdm(total=len(rows), desc=f"cdf-patch:{group}/{method}", unit="img", file=sys.stdout, dynamic_ncols=True)
    ...
    if args.with_regions:
        payload["region_labels"] = np.concatenate(region_chunks, axis=0).astype(np.int16, copy=False)
