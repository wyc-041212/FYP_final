#!/bin/bash
#SBATCH --job-name=celebdf_real_clip
#SBATCH --output=celebdf_real_clip_%j.out
#SBATCH --error=celebdf_real_clip_%j.err
...
if [ -z "$PACK_ROOT_CANDIDATE" ] && [ -n "${SLURM_SUBMIT_DIR:-}" ]; then
  if [ -f "${SLURM_SUBMIT_DIR}/run_celebdf_real_cls_batch.sh" ] && \
     [ -f "${SLURM_SUBMIT_DIR}/run_celebdf_real_patch_batch.sh" ] && \
...
if [ -z "$DATA_ROOT" ]; then
  echo "Set DATA_ROOT to the absolute path of Celeb-DF-v2 on the cluster." >&2
  exit 1
fi
...
if [ -z "$TMP_WORK_ROOT" ]; then
  TMP_WORK_ROOT="$TMP_BASE/celebdf_real_clip"
fi
...
if [ -z "$MANIFEST_CACHE_ROOT" ]; then
  if [ "$RUN_CLS" = "1" ]; then
    MANIFEST_CACHE_ROOT="$TMP_CLS_CACHE_ROOT"
  else
...
if [ "$RUN_CLS" = "1" ] && [ ! -f "$SCRIPT_DIR/run_celebdf_real_cls_batch.sh" ]; then
  echo "Missing $SCRIPT_DIR/run_celebdf_real_cls_batch.sh" >&2
  exit 1
fi

if [ "$RUN_PATCH" = "1" ] && [ ! -f "$SCRIPT_DIR/run_celebdf_real_patch_batch.sh" ]; then
  echo "Missing $SCRIPT_DIR/run_celebdf_real_patch_batch.sh" >&2
  exit 1
fi
...
echo "[INFO] datasets_count=${#DATASETS[@]}"
...
if [ "$RUN_CLS" = "1" ]; then
  ...
  bash "$SCRIPT_DIR/run_celebdf_real_cls_batch.sh" "${DATASETS[@]}"
fi
...
if [ "$RUN_PATCH" = "1" ]; then
  ...
  bash "$SCRIPT_DIR/run_celebdf_real_patch_batch.sh" "${DATASETS[@]}"
fi
