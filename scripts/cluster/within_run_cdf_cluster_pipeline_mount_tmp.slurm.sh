#!/bin/bash
#SBATCH --job-name=within_cdf_mount
...
REMOTE_TMP_SPEC=":sftp,host=${SOURCE_NODE},user=f2256768,key_file=${KEY_FILE}:/tmp"
REMOTE_TMP_MOUNT="${REMOTE_TMP_MOUNT:-/tmp/gpu10tmp_mount}"
...
cleanup() {
  if mountpoint -q "${REMOTE_TMP_MOUNT}"; then
    fusermount3 -u "${REMOTE_TMP_MOUNT}" || true
  fi
}
trap cleanup EXIT
...
check_file() {
  local path="$1"
  log_step "check ${path}"
  timeout 120 bash -lc "test -f \"$path\""
  echo "[OK] ${path}"
}

mkdir -p "${REMOTE_TMP_MOUNT}"
if ! mountpoint -q "${REMOTE_TMP_MOUNT}"; then
  log_step "mount ${SOURCE_NODE}:/tmp -> ${REMOTE_TMP_MOUNT}"
  "${RCLONE_BIN}" mount "${REMOTE_TMP_SPEC}" "${REMOTE_TMP_MOUNT}" --daemon --vfs-cache-mode minimal
  sleep 5
fi

if ! mountpoint -q "${REMOTE_TMP_MOUNT}"; then
  echo "Failed to mount ${SOURCE_NODE}:/tmp at ${REMOTE_TMP_MOUNT}" >&2
  exit 1
fi
...
CDF_PATCH_CACHE_ROOT="${CDF_PATCH_CACHE_ROOT:-${REMOTE_TMP_MOUNT}/f2256768/cdf_cache/cache_clip_patch}"
CDF_SPLIT_NAME="${CDF_SPLIT_NAME:-DF40_test_cdf}"
COMPACT_CACHE_DIR="${COMPACT_CACHE_DIR:-${REMOTE_TMP_MOUNT}/f2256768/fyp_final_compact_cdf}"

REAL_CLS_ROOT="${REAL_CLS_ROOT:-${REMOTE_TMP_MOUNT}/celebdf_real_clip/cache_clip/Celeb-DF-v2/real}"
REAL_COMPACT_DIR="${REAL_COMPACT_DIR:-${REMOTE_TMP_MOUNT}/f2256768/fyp_final_compact_celebdf_real}"
...
echo "[INFO] remote_tmp_mount=${REMOTE_TMP_MOUNT}"
...
echo "[INFO] real_cls_root=${REAL_CLS_ROOT}"
echo "[INFO] real_compact_dir=${REAL_COMPACT_DIR}"
echo "[INFO] output_dir=${OUTPUT_DIR}"

check_file "${CDF_PATCH_CACHE_ROOT}/DF40_test_cdf/EFS/patch_DiT.npz"
check_file "${COMPACT_CACHE_DIR}/tmp__f2256768__cdf_cache__cache_clip_patch__DF40_test_cdf__EFS__patch_DiT.npz.npz"
check_file "${REAL_CLS_ROOT}/cls_Celeb-real.npz"
check_file "${REAL_COMPACT_DIR}/tmp__celebdf_real_clip__cache_clip_patch__Celeb-DF-v2__real__patch_Celeb-real.npz.npz"
...
log_step "run full threshold sweep cdf"
...
log_step "run no_fr threshold sweep cdf"
conda run --no-capture-output -n "${ENV_NAME}" python "${PROJECT_ROOT}/scripts/within_run_cdf_threshold_sweep.py" \
...
log_step "run full cdf replay eval"
...
log_step "run no_fr cdf replay eval"
conda run --no-capture-output -n "${ENV_NAME}" python "${PROJECT_ROOT}/scripts/within_run_cdf_replay_eval.py" \
  --upstream-checkpoint "${NOFR_UPSTREAM}" \
  --patch-branch "${NOFR_PATCH}" \
  --pair-branch "${NOFR_PAIR}" \
...
echo "[DONE] within CDF mount pipeline finished."
