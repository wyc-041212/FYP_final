#!/bin/bash
#SBATCH --job-name=fyp_cdf_pipeline
...
SSH_CMD=(ssh -F /dev/null -o BatchMode=yes)
...
FULL_PATCH="${FULL_PATCH:-${PROJECT_ROOT}/checkpoints/downstream/patch_branch.joblib}"
FULL_PAIR="${FULL_PAIR:-${PROJECT_ROOT}/checkpoints/downstream/pair_branch_full_no_bg_keep_hair.joblib}"
FULL_HEAD="${FULL_HEAD:-${PROJECT_ROOT}/checkpoints/heads/route_meta_head_full_patch_normal_pair_nobg.joblib}"

NOFR_UPSTREAM="${NOFR_UPSTREAM:-${PROJECT_ROOT}/checkpoints/upstream/checkpoint_no_fr.pt}"
NOFR_PATCH="${NOFR_PATCH:-${PROJECT_ROOT}/checkpoints/downstream/patch_branch_no_fr.joblib}"
NOFR_PAIR="${NOFR_PAIR:-${PROJECT_ROOT}/checkpoints/downstream/pair_branch_no_fr_no_bg_keep_hair.joblib}"
NOFR_HEAD="${NOFR_HEAD:-${PROJECT_ROOT}/checkpoints/heads/route_meta_head_no_fr_patch_normal_pair_nobg.joblib}"
...
conda run --no-capture-output -n "${ENV_NAME}" python "${PROJECT_ROOT}/scripts/run_cdf_threshold_sweep.py" \
...
conda run --no-capture-output -n "${ENV_NAME}" python "${PROJECT_ROOT}/scripts/run_cdf_replay_eval.py" \
  --upstream-checkpoint "${FULL_UPSTREAM}" \
  --patch-branch "${FULL_PATCH}" \
  --pair-branch "${FULL_PAIR}" \
  --route-meta-head "${FULL_HEAD}" \
...
echo "[DONE] CDF cluster pipeline finished."
