...
OUTPUT_TAG="${OUTPUT_TAG:-within_no_fr_with_fr}"
...
conda run --no-capture-output -n "${ENV_NAME}" python "${PROJECT_ROOT}/scripts/within_run_cdf_threshold_sweep.py" \
  ...
  --output-json "${OUTPUT_DIR}/${OUTPUT_TAG}_threshold_sweep_cdf.json" \
  --output-csv "${OUTPUT_DIR}/${OUTPUT_TAG}_threshold_sweep_cdf.csv" \
  "${COMMON_SWEEP_ARGS[@]}"
...
conda run --no-capture-output -n "${ENV_NAME}" python "${PROJECT_ROOT}/scripts/within_run_cdf_replay_eval.py" \
  ...
  --head-meta "${NOFR_HEAD_META}" \
  --cdf-groups EFS FR FS \
  --output-json "${OUTPUT_DIR}/${OUTPUT_TAG}_cdf_replay_eval.json" \
  --output-csv "${OUTPUT_DIR}/${OUTPUT_TAG}_cdf_replay_eval.csv" \
  "${COMMON_REPLAY_ARGS[@]}"

echo "[DONE] within no_fr with FR CDF pipeline finished."
