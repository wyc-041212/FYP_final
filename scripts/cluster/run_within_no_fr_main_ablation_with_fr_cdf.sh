...
OUTPUT_TAG="${OUTPUT_TAG:-within_no_fr_main_ablation_with_fr_cdf_cpu_gpu10}"
DEVICE="${DEVICE:-cpu}"
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/within_outputs}"
THRESHOLD_JSON="${THRESHOLD_JSON:-${OUTPUT_DIR}/no_fr_main_ablation_line.json}"
...
echo "[INFO] output_tag=${OUTPUT_TAG}"
echo "[INFO] device=${DEVICE}"
echo "[INFO] cdf_groups=EFS FR FS"
...
FIXED_THRESHOLD_ARGS=()
if [ -n "${FIXED_THRESHOLD}" ]; then
  FIXED_THRESHOLD_ARGS=(--fixed-threshold "${FIXED_THRESHOLD}")
fi

conda run --no-capture-output -n "${ENV_NAME}" python "${PROJECT_ROOT}/scripts/within_run_main_ablation_cdf_only.py" \
  --upstream-checkpoint "${PROJECT_ROOT}/checkpoints/upstream/checkpoint_no_fr.pt" \
  --patch-branch "${PROJECT_ROOT}/within_checkpoints/downstream/patch_branch_no_fr.joblib" \
  --pair-branch "${PROJECT_ROOT}/within_checkpoints/downstream/pair_branch_no_fr.joblib" \
  --route-meta-head "${PROJECT_ROOT}/within_checkpoints/heads/route_meta_head_no_fr.joblib" \
  --head-meta "${PROJECT_ROOT}/within_checkpoints/heads/route_meta_head_no_fr_meta.json" \
  ...
  --output-json "${OUTPUT_DIR}/${OUTPUT_TAG}.json" \
  --output-csv "${OUTPUT_DIR}/${OUTPUT_TAG}.csv"

echo "[DONE] within no_fr main ablation with FR CDF finished."
