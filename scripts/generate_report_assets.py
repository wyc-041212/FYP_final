...
def build_main_results_assets() -> None:
    full_local = read_threshold_csv(ROOT / "outputs" / "full_threshold_sweep.csv")
    no_local = read_threshold_csv(ROOT / "outputs" / "no_fr_threshold_sweep.csv")
    full_cdf_real, full_cdf_fake = read_cdf_csv(ROOT / "outputs" / "full_threshold_sweep_cdf.csv")
    no_cdf_real, no_cdf_fake = read_cdf_csv(ROOT / "outputs" / "no_fr_threshold_sweep_cdf.csv")
    no_exfr = nofr_ff_exfr_map(ROOT / "outputs" / "no_fr_threshold_sweep.json")
    no_ood_exfr = nofr_ood_exfr_map(ROOT / "outputs" / "no_fr_threshold_sweep.json")
...
def build_ablation_assets() -> None:
    rows = []
    for setting, path in [
        ("Full corrected", ROOT / "outputs" / "full_main_ablation_line.csv"),
        ("no-FR corrected", ROOT / "outputs" / "no_fr_main_ablation_line.csv"),
    ]:
    ...
    full_data = {r["model"]: r for r in read_csv_rows(ROOT / "outputs" / "full_main_ablation_line.csv") if r["split"] == "ood"}
    full_cdf = {r["model"]: r for r in read_csv_rows(ROOT / "outputs" / "full_main_ablation_line.csv") if r["split"] == "cdf_real_subset"}
    no_data = {r["model"]: r for r in read_csv_rows(ROOT / "outputs" / "no_fr_main_ablation_line.csv") if r["split"] == "ood"}
    no_cdf = {r["model"]: r for r in read_csv_rows(ROOT / "outputs" / "no_fr_main_ablation_line.csv") if r["split"] == "cdf_real_subset"}

    rows_csv: list[dict[str, str]] = []
    for setting, base_path, ood_map, cdf_map in [
        ("Full corrected", ROOT / "outputs" / "full_main_ablation_line.csv", full_data, full_cdf),
        ("no-FR corrected", ROOT / "outputs" / "no_fr_main_ablation_line.csv", no_data, no_cdf),
    ]:
...
def build_threshold_figure() -> None:
    full_local = read_threshold_csv(ROOT / "outputs" / "full_threshold_sweep.csv")
    no_local = read_threshold_csv(ROOT / "outputs" / "no_fr_threshold_sweep.csv")
    full_cdf_real, _ = read_cdf_csv(ROOT / "outputs" / "full_threshold_sweep_cdf.csv")
    no_cdf_real, _ = read_cdf_csv(ROOT / "outputs" / "no_fr_threshold_sweep_cdf.csv")
    no_exfr = nofr_ff_exfr_map(ROOT / "outputs" / "no_fr_threshold_sweep.json")
    no_ood_exfr = nofr_ood_exfr_map(ROOT / "outputs" / "no_fr_threshold_sweep.json")
